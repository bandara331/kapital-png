  ...
  const handleScan = async () => {
    ...
    try {
      const res = await fetch("/api/ocr", {
        ...
        body: JSON.stringify({ receiptText: MOCK_RECEIPT_TEXT }),
      });
        ...
        >
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-[48px] items-center">
            ...
            <div className="relative">
              <div
                className={`relative border-2 border-dashed rounded-[16px] flex flex-col items-center justify-center p-[40px] transition-colors duration-300 min-h-[360px] overflow-hidden ${
                  scanState === "idle"
                ...
                <AnimatePresence mode="wait">
                  {scanState === "idle" && (
                    ...
                    >
                      ...
                      <h3 className="font-[family-name:var(--font-space-grotesk)] font-semibold text-[18px] mb-[8px]">Drop a receipt to scan</h3>
                      <p className="text-[14px] text-[color:var(--muted)] mb-[24px]">or click "Test Scan" to see Groq AI in action</p>
                      <button className="inline-flex items-center gap-2 font-[family-name:var(--font-space-grotesk)] font-semibold text-[14.5px] px-[22px] py-[10px] rounded-full transition-all duration-200 bg-[color:var(--color-teal)] text-[color:var(--color-navy-3)] hover:-translate-y-[2px] shadow-[0_4px_14px_rgba(47,174,147,0.25)]">
                        Test Scan
                      </button>
                    </motion.div>
                  )}
                  ...
                  {(scanState === "scanning" || scanState === "complete" || scanState === "error") && (
                    <motion.div
                      ...
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      className="absolute inset-0 flex items-center justify-center bg-[color:var(--card)]"
                    >
                      {/* CSS Wireframe Receipt */}
                      <div className="relative w-[220px] bg-white border border-[color:var(--color-line)] shadow-lg pt-6 pb-8 px-5 font-mono text-[10px] text-[color:var(--color-ink)] opacity-70">
                        ...
                        {/* Laser line — only animate while scanning */}
                        {scanState === "scanning" && (
                          <motion.div
                            initial={{ top: "0%" }}
                            animate={{ top: "100%" }}
              ...
              <AnimatePresence mode="wait">
                ...
                {scanState === "complete" && extractedData && (
                  <motion.div
                    ...
                    initial={{ opacity: 0, x: -20 }}
                    animate={{ opacity: 1, x: 0 }}
                    className="bg-[color:var(--color-navy)] text-white rounded-[16px] p-[32px] shadow-lg relative overflow-hidden"
                  >
                    ...
                    <div className="flex items-center gap-[10px] mb-[24px]">
                      ...
                      <div>
                        ...
                        <p className="text-white/40 text-[11px] mt-0.5">via Groq · Llama 3 · 8B</p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-[16px] font-mono text-[14px]">
                      {Object.entries(extractedData).map(([key, value], i) => (
                        ...
                        >
                          ...
                          <span className={key === "total" ? "text-[color:var(--color-gold)] font-bold text-[16px]" : "text-[color:var(--color-teal-2)]"}>
                            {value}
                          </span>
                        </motion.div>
                      ))}
